		...
		while (conversion == false) {
			...
			for (int v = 0; v < m; v++) {
				...
				while (ptsIterator2.hasNext()) {
					...
					for (int i = 0; i < l; i++) {
						for (int a = 0; a < m; a++) {
							if (ptsArrList2.get(i).equals(a)) { //X(i) = a
								cva += g.beta[i][i][v] * g.e(g.nonTerminals.get(v), (char)a);
							}
						}
					}
				}
			...
			And the input is g and pre_g =, instead eva and previous eva
			*/
			float eva = 0;
			float tvyz = 0;
